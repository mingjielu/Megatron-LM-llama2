...
class MLPSubmodules:
    ...
    linear_fc1_lora_A: Union[ModuleSpec, type] = None
    linear_fc1_lora_B: Union[ModuleSpec, type] = None

    linear_fc2: Union[ModuleSpec, type] = None
    linear_fc2_lora_A: Union[ModuleSpec, type] = None
    linear_fc2_lora_B: Union[ModuleSpec, type] = None
...
class MLP(MegatronModule):
    ...
    def __init__(
        ...
        if 0 and self.config.lora_alpha >0   and self.config.lora_rank > 0:
            ...
            lora_a_layer, lora_b_layer, parallel_mode_a, parallel_mode_b = get_lora_linear_type(self.linear_fc1)
            self.linear_fc1_lora_A = build_module(
                lora_a_layer,
                self.input_size,
                ...
                tp_comm_buffer_name='fc1_a',
            )
            self.linear_fc1_lora_B = build_module(
                lora_b_layer,
                self.config.lora_rank,
                ffn_hidden_size,
                config=self.config,
                ...
                tp_comm_buffer_name='fc1_b',
            )

            lora_a_layer, lora_b_layer, parallel_mode_a, parallel_mode_b = get_lora_linear_type(self.linear_fc2)
            self.linear_fc2_lora_A = build_module(
                lora_a_layer,
                self.config.ffn_hidden_size,
                self.config.lora_rank,
                config=self.config,
                ...
                is_expert=is_expert,
                parallel_mode = parallel_mode_a,
                tp_comm_buffer_name='fc2_a',
            )
            self.linear_fc2_lora_B = build_module(
                lora_b_layer,
                self.config.lora_rank,
                self.config.hidden_size,
                config=self.config,
                ...
                is_expert=is_expert,
                parallel_mode = parallel_mode_b,
                tp_comm_buffer_name='fc2_b',
            )

            self.linear_fc1.weight.requires_grad = False
            if self.linear_fc1.bias is not None:
                self.linear_fc1.bias.requires_grad = False
            if hasattr(self.linear_fc1, 'layer_norm_weight'):
                self.linear_fc1.layer_norm_weight.requires_grad = False

            self.linear_fc2.weight.requires_grad = False
            if self.linear_fc2.bias is not None:
                self.linear_fc2.bias.requires_grad = False
            if hasattr(self.linear_fc2, 'layer_norm_weight'):
                self.linear_fc2.layer_norm_weight.requires_grad = False
    ...
    def forward(self, hidden_states):
        ...
        if self.lora_enabled:
            tmp, _ = self.linear_fc1_lora_A(hidden_states)
            tmp, _ = self.linear_fc1_lora_B(tmp)
            intermediate_parallel = intermediate_parallel + self.lora_alpha*tmp
        ...
        if self.lora_enabled:
            tmp, _ = self.linear_fc2_lora_A(intermediate_parallel)
            tmp, _ = self.linear_fc2_lora_B(tmp)
