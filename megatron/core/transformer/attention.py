...
from megatron.core.extensions.transformer_engine import get_lora_linear_type
...
class SelfAttentionSubmodules:
    ...
    linear_qkv_lora_A: Union[ModuleSpec, type] = None
    linear_qkv_lora_B: Union[ModuleSpec, type] = None

    core_attention: Union[ModuleSpec, type] = None

    linear_proj: Union[ModuleSpec, type] = None
    linear_proj_lora_A: Union[ModuleSpec, type] = None
    linear_proj_lora_B: Union[ModuleSpec, type] = None
...
class CrossAttentionSubmodules:
    ...
    linear_q_A: Union[ModuleSpec, type] = None
    linear_q_B: Union[ModuleSpec, type] = None

    linear_kv: Union[ModuleSpec, type] = None
    linear_kv_lora_A: Union[ModuleSpec, type] = None
    linear_kv_lora_B: Union[ModuleSpec, type] = None
...
class Attention(MegatronModule, ABC):
    ...
    def __init__(
        ...
        if 0 and self.config.lora_alpha >0   and self.config.lora_rank > 0:
            self.lora_enabled = True
            self.lora_alpha = self.config.lora_alpha
            lora_a_layer, lora_b_layer, parallel_mode_a, parallel_mode_b = get_lora_linear_type(self.linear_proj)

            self.linear_proj_lora_A = build_module(
                lora_a_layer,
                self.query_projection_size,
                ...
                skip_bias_add=False,
                is_expert=False,
                parallel_mode = parallel_mode_a,
                tp_comm_buffer_name='proj_a',
            )
            self.linear_proj_lora_B = build_module(
                lora_b_layer,
                self.config.lora_rank,
                self.config.hidden_size,
                config=self.config,
                ...
                skip_bias_add=False,
                is_expert=False,
                parallel_mode = parallel_mode_b,
                tp_comm_buffer_name='proj_b',
            )
            self.linear_proj.weight.requires_grad = False
            if self.linear_proj.bias is not None:
                self.linear_proj.bias.requires_grad = False
            if hasattr(self.linear_proj, 'layer_norm_weight'):
                self.linear_proj.layer_norm_weight.requires_grad = False
    ...
    def forward(
        ...
        if self.lora_enabled:
            tmp, _ = self.linear_proj_lora_A(core_attn_out)
            tmp, _ = self.linear_proj_lora_B(tmp)
            output = output + self.lora_alpha*(tmp)
        return output, bias
...
class SelfAttention(Attention):
    ...
    def __init__(
        ...
        if 0 and self.config.lora_alpha >0   and self.config.lora_rank > 0:
            self.lora_enabled = True
            self.lora_alpha = self.config.lora_alpha
            lora_a_layer, lora_b_layer, parallel_mode_a, parallel_mode_b = get_lora_linear_type(self.linear_qkv)

            self.linear_qkv_lora_A = build_module(
                lora_a_layer,
                self.config.hidden_size,
                self.config.lora_rank,
                config=self.config,
                ...
                is_expert=False,
                parallel_mode = parallel_mode_a,
                tp_comm_buffer_name='qkv_a',
            )

            self.linear_qkv_lora_B = build_module(
                lora_b_layer,
                self.config.lora_rank,
                self.query_projection_size + 2 * self.kv_projection_size,
                config=self.config,
                ...
                is_expert=False,
                parallel_mode = parallel_mode_b,
                tp_comm_buffer_name='qkv_b',
            )
            self.linear_qkv.weight.requires_grad = False
            if self.linear_qkv.bias is not None:
                self.linear_qkv.bias.requires_grad = False
            if hasattr(self.linear_qkv, 'layer_norm_weight'):
                self.linear_qkv.layer_norm_weight.requires_grad = False
    ...
    def get_query_key_value_tensors(self, hidden_states, key_value_states=None):
        ...
        if self.lora_enabled:
            tmp, _ = self.linear_qkv_lora_A(hidden_states)
            tmp, _ = self.linear_qkv_lora_B(tmp)
            mixed_qkv = mixed_qkv + self.lora_alpha*(tmp)
